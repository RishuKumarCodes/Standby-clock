...
const hexToRgba = (hex: number, opacity: number, intensity: number = 0.8) => {
  let r = 0,
    g = 0,
    b = 0;
  if (hex.length === 4) {
...
const VARIANT_CONFIG: Record<ClockVariant, { elementGap: number }> = {
  full: { elementGap: 20 },
  themeCard: { elementGap: 5 },
  smallPreview: { elementGap: 3 },
  colorSettings: { elementGap: 6 },
};

export default function CircleTheme({
  color = "#000",
  variant = "full",
}: ThemeProps) {
  const { elementGap } = VARIANT_CONFIG[variant];
